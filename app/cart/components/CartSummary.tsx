...
export default function CartSummary({ selectedIds }: Props) {
  ...
  const subTotal = selectedItems.reduce((sum, item) => {
    const salePrice = formatCurrencyToNumber(
      item.sale_price ?? item.inventory?.sale_price ?? "0"
    );
    const offerPrice = formatCurrencyToNumber(
      ...
      <div className="mb-6">
        ...
        <ul className="space-y-4 max-h-48 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
          {selectedItems.map((item) => {
            ...
            const image =
              item.image ?? item.inventory?.image ?? "/fallback.jpg";
            const salePrice = formatCurrencyToNumber(
              item.sale_price ?? item.inventory?.sale_price ?? "0"
            );
            const offerPrice = formatCurrencyToNumber(
            ...
            return (
              <li
                key={item.id}
                className="flex gap-4 items-center py-2 border-b border-gray-100 last:border-b-0"
              >
